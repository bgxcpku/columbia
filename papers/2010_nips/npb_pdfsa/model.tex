...
\end{eqnarray*}
%  
where $H$ in our case is a geometric distribution over the integers with parameter $\lambda$.   The resulting hierarchical model becomes the well known hierarchical Dirichlet process (HDP) over a discrete alphabet \cite{Teh2006}.
Given a finite amount of data, there can only be nonzero counts for a finite number of state/symbol pairs, so we can marginalize out all but at most $T$ elements of $\delta$.  Denote the remaining non-marginalized out entries by $\delta^+$ as before.  We can sample the elements of $\delta^+$ as before using \eqref{delta:mh} provided that we can propose from the HDP (i.e. sample from the  $|Q|\rightarrow\infty$ limit of \eqref{delta:pred_marg}).  In any of a number of HDP sampler representations this is easy to do.  We use the Chinese restaurant franchise representation \cite{Teh2006} in which posterior predictive distribution of $\delta_{ij}$ given $\delta_{-ij}^+$ can be expressed with $\phi_j$ and $\bmu$ integrated out as

\begin{equation}
P(\delta_{ij} = q_{i'}|\delta_{-ij}^+,\alpha, \gamma) = \frac{v_{i'j}}{\alpha + D^+_j } + \frac{\alpha}{\alpha + D^+_j}\left(\frac{w_{i'}}{\gamma+w_\cdot} + \frac{\gamma}{\gamma+w_\cdot}H(q_{i'})\right). \label{delta:pred_marg}
\end{equation}

Here $w_{i'}$ and $w_\cdot = \sum_i w_i$ are bookkeeping counts required by the Chinese Restaurant franchise sampler.  These counts are themselves stochastic and must themselves be sampled using the instructions provided in \cite{Teh2006}.  
