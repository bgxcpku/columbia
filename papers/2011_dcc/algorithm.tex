...
an entropy encoder exists that takes $F(\textsc{prev}(s))$ and $F(s)$ as arguments and returns a bit stream \citep{Witten1987}. %  The cumulative distribution function is well defined because the symbols are ordered. 
We use the function \textsc{prev}$(s)$ to refer to the symbol previous to $s$ in the symbol set $\Sigma$ and define $F(\textsc{prev}(\sigma_0)) = 0$.  %Note that the symbol set can be ordered as the set is discovered.
...
%		\State $d \la $ \textsc{GetDiscount}(\N)		
		...
		\Else
			\State $\pi \la (1- d^{\N} m)\pi + d^{\N} m P_{\xi([])}$
			
%			\mathcal{U}_{\Sigma}$ %\Comment{$\mathcal{U}(\Sigma)$ is the uniform distribution over $\Sigma$}
%			\State \Return $\pi$
		\EndIf 
	\EndFunction
		\end{algorithmic}
...
\begin{algorithm}[H]
	...
	%	\State $d \la$ \textsc{GetDiscount}$(\N)$
		\While{$c^\N > k$}
			...
			\State $\sigma \la $ \textsc{Multinomial}$(\pi)$ %s.t. $\pi_l = \frac{c_l}{c}$ % \Comment{$\pi$ is a distribution over $\Sigma$}
