...
\problem{65}{
	...
	Every variable is binary and so when passing messages the first element of the column vector should be proportional to the probability of TRUE and the second number proportional to the probability of FALSE.  The network you are implementing and the appropriate probability tables can be found in Figure~\ref{bnexample}. The functions you are responsible for filling out are :
	...
	\begin{enumerate}
	 ...
	 The graphical structure is set up in main\_alarm\_network.m.  At the bottom of that file you can see an example of inference in the model after setting the values of vn\_e and vn\_b to one.  \\
	 \begin{figure*}
		\begin{center}
...
\problem{35}{
	...
	The first task in this section of the HW will be to understand the code.  Then, you will need to implement the method {\bf getMessage(obj, to\_unid)} in both the factor\_node and variable\_node objects.  This will take some creativity and may require you to use a recursive function of make use of the eval function in MATLAB. Finally, you need to implement the method {\bf getMarginalDistribution(obj)} in the variable\_node class definition. \\
