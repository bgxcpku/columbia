...
\begin{quote}
The assignments were challenging and probably the best part of the course. It was with bated breath, and nervous anticipation - almost like a blind date with someone you knew would be pretty - that we waited for the homeworkÉ ok, maybe that was 10\% exaggerated. ....
\end{quote}
\begin{quote}
...
\end{quote}
\begin{quote}
Very difficult programming assignments, and I learned a lot doing them, É
\end{quote}
...
\begin{itemize}
\item Government Bias in sub-Saharan African Press
\begin{itemize}
\item Bayesian logistic regression to predict ``slant'' of press articles from extracted text features
\end{itemize}
\item A Pattern Recognition System for American Sign Language
\begin{itemize}
\item Component analysis approach to feature extraction and classification of ASL from {\em videos} of expert signers
\end{itemize}
\item Supervised Topic Modeling in Clinical Text
\begin{itemize}
\item Automatically assign ICD-9-CM codes to patient discharge records.
\end{itemize}
\item FriendFinder: Predicting missing links in a Gargantuan social network
\item Ordering ShakespeareÕs Plays using a Sequential Generative Model
\end{itemize}
}
