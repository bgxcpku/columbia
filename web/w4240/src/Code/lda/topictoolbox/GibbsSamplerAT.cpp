...
void GibbsSamplerAT( double ALPHA, double BETA, int W, int T, int D, int NN, int OUTPUT, int n, int *z, int *d, int *w, int *x, int *wp, int *at, int *ztot, int *atot, int *order, double *probs, int *irad, int *jcad, int startcond )
{
  ...
  if (startcond==1) {
      ...
      for (i=0; i<n; i++)
      {
          ...
          if (nauthors==1)
              ...
              i_pick = i_start + (int) ( (double) randomMT() * (double) nauthors / (double) (4294967296 + 1.0) );
  ...
  if (startcond==0) {
      ...
      for (i=0; i<n; i++)
      {
          ...
          topic = (int) ( (double) randomMT() * (double) T / (double) (4294967296 + 1.0) );
  ...
  for (i=0; i<(n-1); i++) {
      ...
      rp = i + (int) ((double) (n-i) * (double) randomMT() / (double) (4294967296 + 1.0));
  ...
  for (iter=0; iter<NN; iter++) {
      ...
      for (ii = 0; ii < n; ii++) {
          ...
          r = (double) totprob * (double) randomMT() / (double) 4294967296;
          max = probs[0];
          if (nauthors==1) {
              topic = 0;
              while (r>max) {
