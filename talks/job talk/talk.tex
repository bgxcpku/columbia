...
\item Worst case linear time estimation
\item Worst case constant time inference
\item Worst case constant space memory complexity
\end{itemize}
Problems
\begin{itemize}
...
\item Worst case linear time estimation  $\checkmark$
\item Worst case constant time inference  $\checkmark$
\item Worst case constant space memory complexity  $\checkmark$
\end{itemize}
Problems
\begin{itemize}
...
\begin{frame}[t]{Steps}
\begin{itemize}
\item Incrementally estimate $\mathsf{P},$ a {\em joint} distribution over sequences of unbounded length in worst case linear time from a single observed sequence.
%\begin{itemize}
...
\begin{quote}The Turing test for artificial intelligence is widely accepted, but is subjective, qualitative, non-repeatable, and difficult to implement. An alternative test without these drawbacks is to insert a machine's language model into a predictive encoder and compress a corpus of natural language text. A ratio of 1.3 bits per character or less indicates that the machine has AI.\end{quote}
 ...
 \frame {%slide 4
 \frametitle{Sampling From Trained Model}
