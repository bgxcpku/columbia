...
\defcitealias{Wood_NIPS_2012}{W.,2011}
...
\title[Reusable Models] 
{
Reusable Models for Statistical Machine Learning
...
\author[Wood]
{
  Frank~Wood\\
  ~\\%\inst{1}\\
  \tiny{Joint work with Jonathan Huggins, Willie Neiswanger, Nicholas Bartlett, Mike Dewar}%, \\Jan Gasthaus, Yee Whye Teh, Lancelot James, and Cedric Archambeau}
}
...
\frame[t] {
\frametitle{Statistical Machine Learning} % \only<2>{\alert{Research}}}
...
\item {Model} 
\item Data 
\item  \only<1>{Inference algorithm}
\end{itemize}
\item Computation
\end{itemize}
\end{block}
...
\frame[t] {
\frametitle{This Talk : {\em One} New Reusable Model}
...
%\end{block}
%\uncover<2>{
\begin{block}{Infinite structured explicit duration HMM  \citepalias{Huggins-2012-AISTATS} }
\begin{itemize}
...
\begin{figure}[t]
		\begin{center}
			\includegraphics[width = \textwidth, transparent]{HMM-graphical-model-no-fonts}
		\end{center}
	\end{figure}
...
Hidden Markov models (HMMs) \citep{Rabiner:1989} are used in many data exploration and engineering applications :
\newline
...
\item State dwell times are not usually geometrically distributed
%\eqan
...
\frame[t] {
\frametitle{Infinite Structured Explicit Duration HMM \citepalias{Huggins-2012-AISTATS} }

Characteristics :  
\begin{itemize}
\item Infinite state cardinality % (Bayesian nonparametric)
\item Explicitly parameterisable duration distributions
\item Structured transitions
\end{itemize}
Key Insight :  
\begin{itemize}
\item States live in an auxiliary space
\item Each state may transition only to states in a nearby region 
\end{itemize}
...
%\[P(\mathbf z | \mathbf x) = \frac{P(\mathbf z, \mathbf x)}{P(\mathbf x)} = \frac{P(\mathbf x| \mathbf z)P(\mathbf z)}{P(\mathbf x)}\]
%%Learning
%%\[\argmax_\theta P(\theta | \mathbf x) = \argmax_\theta \sum_{\mathbf z} P(\theta, \mathbf z | \mathbf x) =  \argmax_\theta \sum_{\mathbf z}  \frac{P(\mathbf z, \mathbf x, \theta)}{P(\mathbf x)} \]
%\begin{itemize}
%\item  $\mathbf z$ : Latent variables
...
%\item Asymptotic convergence characteristics not understood
%\end{itemize}
%\end{itemize}
%}
%
...
\item Problem : *
\item Data : *
\item Latent variables : state cardinality, sequence, and characteristics 
\begin{itemize}
...
\item Inference algorithm : Block Gibbs with forward-backward slice-sampling \citepalias{Dewar:2011},\citep{VanGael:2008}
\end{itemize}
\pause
\textbf{Nearest parametric comparables}
\begin{itemize}
\item Explicit duration HMM / Hidden Semi-Markov Model \\
\citep{Mitchell95,Murphy02,Yu:2010}
\end{itemize}
\pause
\textbf{Nearest Bayesian nonparametric comparables}
\begin{itemize}
\item Infinite HMM  \citep{Beal:2002}
...
%\begin{frame}[t]{Conjectures and Opinions}
%\begin{itemize}
%\item Bayesian inference is fundamentally intractable
%\begin{itemize}
%\item Except
%\begin{itemize}
%\item In a few analytically tractable models
%\item And in models with few parameters
%\end{itemize}
%\end{itemize}
%\item Samplers are probably better characterized as stochastic, gradient-ascent, MAP-approximation algorithms
%\begin{itemize}
%%\item Mode switching uncommon
%\item Mixing over the full posterior of large, complex models never happens in practice
%\end{itemize}
%\item Bayesian nonparametric models are a dead-end
%\begin{itemize}
%\item No scalability in the absence of sufficiency \citepalias{Wood_NIPS_2012}
%\end{itemize}
%\end{itemize}
%\end{frame}

%\begin{frame}[t]{Going-Forward Research Philosophy}
%\begin{itemize}
%\item Justification for Bayesian approach
%\begin{itemize}
%\item Results
%\begin{itemize}
%\item Lossless compression \citepalias{Gasthaus2010}
%\item Reward accrual in reinforcement learning \citep{Doshi-Velez2009}
%\item $\ldots$
%\end{itemize}
%\end{itemize}
%
%\item Applied research program 
%\begin{itemize}
%\item Reinforcement learning / control
%\item Dimensionality reduction / visualization
%%\item Regularized, highly-expressive latent variable models
%\end{itemize}
%\end{itemize}
%\end{frame}
...
%\begin{frame}[t]{Ideas}
%\begin{itemize}
%\item Posterior inference for process diagram inference from sequential measurements.
%\end{itemize}
%\end{frame}
\frame[t] {
...
\item Near term :
\begin{itemize}
\item Constant space approximations to Bayesian nonparametric models
\begin{itemize}
\item Like \citepalias{Bartlett2011} and \citep{Csat'o2002,Snelson2006}
\end{itemize}
\item ISEDHMMs for simultaneous localization and mapping
\item Tracking goal-directed agents
\end{itemize}

\item Long term : probabilistic control
%\begin{itemize}
%\item Latent variable models {\em necessary}
%%\item Full posterior averaging {\em not necessary}
%%\begin{itemize}
%%\item Regularization is key
%%\end{itemize}
%\item Application performance is goal
%%\begin{itemize}
%%\item Reward accrual
%%\end{itemize}
%\end{itemize}
...
\frame[t] {
\frametitle{Static World}
\vspace{-.5cm}
%\movie[height=1.125in,width=1.5in,poster]{Check out this.}{ddp4mtt_pets2000.mp4}
\begin{figure}[ht]
\includemovie[toolbar,autoclose,poster=gridworld.jpg,text={\small(Click to Play)}]{9cm}{6cm}{gridworld.mp4}
%\caption{Gridworld}
\end{figure}
Latent-space, state-transition region-constraints allow ``localization'' (state sequence) at the same time as ``mapping''  (transition matrix structure)
...
\frame[t] {
\frametitle{Dynamic World}
The world consists of dynamic actors and static background.
%\movie[height=1.125in,width=1.5in,poster]{Check out this.}{ddp4mtt_pets2000.mp4}
\begin{figure}[ht]
\includemovie[toolbar,autoclose,poster=ant_tracking.png,text={\small(Click to Play)}]{9cm}{6cm}{smcddptracking.mp4}
\caption{Multiple target tracking (ANTS) \citepalias{Neiswanger2012}}
\end{figure}
}

\begin{frame}[t]{Future Research}
\vspace{-1cm}
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=7cm]{action_reward_cycle.pdf}
\label{fig:actors}
\end{center}
\end{figure}
Agents attempting to accrue reward through action need highly-expressive, computationally-efficient, probabilistic, unsupervised, latent-variable models to represent and plan using the state of the world.
%\vspace{1cm}
\center{\tiny{Images: Master isolated images, David Castillo Dominici,  / FreeDigitalPhotos.net}}
...
\begin{description}
\item[Funding ] Google, Xerox 
\end{description}
\begin{description}
\item[Collaborators ] Huggins, Neiswanger, Dewar, Bartlett
\end{description}
}
...
\frame[t] {
\frametitle{Diagram Inference / Automated Science}

%\movie[height=1.125in,width=1.5in,poster]{Check out this.}{ddp4mtt_pets2000.mp4}
\begin{figure}[ht]
\includegraphics[width=5.5cm]{activity_diagram.pdf}
\end{figure}
\centering{Diagram inference $\equiv$ ``mapping''}
...
\frame[t]
{
\frametitle{Lossless Compressor Based on the Sequence Memoizer}
