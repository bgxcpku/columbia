...
\begin{frame}[t]{Example : A Simple PYP Model  \cite{Pitman1997}}
Consider
\begin{eqnarray*}
...
\begin{frame}[t]{Outline}
	\begin{block}{Talk Outline}
		\begin{itemize}
			\item Review the generative process for the Pitman-Yor process and its relationship with the Ewen's sampling distribution
			\item Discuss generative process for the Ewen's sampling distribution and its properties
...
\begin{frame}[t]{Pitman-Yor Process}
	\begin{block}{Model}
		\vspace{-.5cm}
		\begin{eqnarray*}
			G | d,c, G_0 &\sim& \PY(d,c,G_0) \\
...
\begin{frame}[t]{Dependent PYP}
	...
	\begin{block}{Generative Process}
		\begin{itemize}
			...
			\item To generate a sample at time $t$, add $n^t$ objects to the partition following to the sequential generative process of the $\ES(d,c)$
...
\begin{frame}[t]{Dependent HPYP}
	\begin{block}{Vocabulary}
		It is often convenient to think of this generative process as a Chinese restaurant where each part of the partition is a table and each object a customer sitting at a table
	\end{block}
...
\begin{frame}[t]{Deletion}
	...
	\begin{block}{Constant Space}
		\begin{itemize}
			\item A generative procedure which deterministically deletes all customers in a chosen leaf restaurant is well specified.  Since empty restaurants are not represented during inference, such a model could be used to limit the number of instantiated restaurants and allow for constant space representation.
			\end{itemize}
	\end{block}
\end{frame}
...
\begin{frame}[t]{Discussion}
	\begin{block}{Future Work}
		\begin{itemize}
			...
			\item More extensive testing with massive data
