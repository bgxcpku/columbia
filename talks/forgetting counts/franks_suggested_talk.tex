...
\begin{frame}[t]{Example: A Hierarchical PYP Model}
	Consider arranging Pitman-Yor processes hierarchically, such as in the HPYP trigram language model
	\[
	\begin{array}{rccl}
	...
	Inferring the posterior distribution requires maintaining counts of all observed values in all observed length 2 contexts.
\end{frame}

\begin{frame}[t]{Goals}
	\begin{block}{Stationary Model for Dependent Data}
\begin{itemize}
...
\end{itemize}
	...
	\end{block}
	\begin{block}{Bounded Memory Inference}
			\begin{itemize}
\item   Constant memory requires that we constrain the model.  
\item The resulting estimator may be inconsistent.  
\item In sparse Gaussian process modeling, forgetting strategies have been used to achieve constant time and space (independent of the number of observations) inference procedures \cite{ Lawrence2003, Csat'o2002, Snelson2006}.
...
\begin{frame}{Consistency of $\ES$ Under Deletion}
	...
	\begin{block}{Application}
		...
		\[
		\begin{array}{rcll}
			\G_1 | d_0, c_0, \G_0 &\sim& \PY(d_0,c_0, \G_0) \\
			\G_2^t | d_1,c_1,\G_1 &\sim& \PY(d_1,c_1,\G_1)& t = 1, \ldots, T\\
			X_i^t | \G_2^t &\sim& \G_2^t& i = 1, \ldots, N_t, t = 1, \ldots, T
		\end{array}
		\]
	\end{block}
\end{frame}
...
\begin{frame}[t]{Dependent HPYP as Constrained Memory}
	\begin{itemize}
		\item{``The number of instantiated nodes can be limited using the deletion scheme developed for dependent HPYPÕs. Memory savings can be achieved by deleting all the elements in a leaf node \ldots " \citet{Bartlett2010}.}
		...
		\item{``Our deletion process requires us to assume the independence of a large number of distributions. While this may seem troubling, a key insight is that the entire set of distributions for which we must make the independence assumption are dependent on an existing node in the tree. Much of the information will be retained in the tree by the remaining node " \citet{Bartlett2010}.}
	\end{itemize}
\end{frame}

\begin{frame}[t]{Intuitions}
	\begin{exampleblock}{Features}
		\begin{itemize}
			\item Generative process allows for time-varying distributions
			\item Number of instantiated nodes can be limited by removing entire restaurants
		\end{itemize}
	\end{exampleblock}
...
\begin{frame}[t]{Constant Space Sequence Memoizer}
	...
	\begin{itemize}
		\item Calgary corpus
		\item Different constant memory approaches
...
\begin{frame}{Motivation: Streams}
